    ...
    begin
       ...
       begin
         ...
         begin
            ...
            {if (s=2) and (t=5) then
            begin
             writeln('spurNode='+IntToStr(spurNode));
             writeln('spurV='+IntToStr(spurV[t]));
             writeln('rootV='+IntToStr(rootV[spurNode]));
             for y:=1 to LLast[kk] do write('-'+IntToStr(totPath[y])+'-');
             writeln('totV='+IntToStr(totV[t]));
            end;}
         end;
         //restore all costs
         for i:=Z.Count-1 downto 0 do
       ...
       {if (s=2) and (t=5) then
       begin
        writeln('kk='+IntToStr(kk));
        writeln('VB['+IntToStr(kk)+']='+IntToStr(VB[kk]));
       end;}
       if VB[kk]=PMV[1] then //if the kth shortest path in heavier than the optimal one, all equivalent path have been found.
       begin
          PPath:=LPMB.Items[kk];
          PMLast[k]:=LLast[kk];
          PMV[k]:=VB[kk];
